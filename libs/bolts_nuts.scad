...
module example() {
    ...
    translate([9,0,0]) nut(diameter=5, pitch=0.8, outer_dia=8, threadType="metric", v=1);
...
module boltHead(head="socket", diameter=3, hex_head_thickness=2, hex_head_dia=5.5, 
                socket_head_dia=5.5, socket_head_thickness=3, socket_tool_size=2.5,
                button_thickness = 1.04, tolerance=0, quality = 24, list=false) {
  ...
  if (head == "socket") { // hex socket head
    ...
    headThick = socket_head_thickness-(1/8 * socket_head_dia)+tolerance;
    headRad = (socket_head_dia + tolerance)/2;
    ...
    translate([0, 0, ((1/8 * socket_head_dia)/2*transZ)-o*5]) 
    difference() {
      ...
      translate([0, 0, headThick/2])
        cylinder( r = hexRadius(socket_tool_size), $fn = 6, h = headThick*.8);
      translate([0, 0, headThick/4])
        cylinder( r1 = 0, r2=hexRadius(socket_tool_size), $fn = 6, h = headThick/4+0.01); 
    } // end difference
  } // end if socket
  ...
  if (head == "conical") {
    // minkwoski() adds 2*sphere radius to the head adjust variables to deal with this
    headThick = socket_head_thickness*1.5 -(1/32*socket_head_dia)/2+tolerance;
    headR2 = (socket_head_dia*.8 -1/32*socket_head_dia)/2+tolerance/2;
    headR1 = (socket_head_dia+tolerance)/2;
    ...
    difference() {
      ...
      translate([0, 0, headThick*.25])
        cylinder(r = hexRadius(socket_tool_size), h = headThick*.8, $fn = 6);
    }
  } // end if conical


  if (head == "hex") {
    headThick = hex_head_thickness+tolerance;
    headRadius = hexRadius(hex_head_dia)+tolerance;
    intersection(size) {
      cylinder (r = headRadius, h = headThick, $fn = 6); // six sided head
      cut_chamfer(height = headThick, boltSize = hex_head_dia+tolerance*2, quality = quality);
    } 
  } // end if hex

  if (head == "flatSocket" || head == "flatHead") { 
    headThick = socket_head_thickness+tolerance;
    headR1 = (diameter+tolerance)/2;
    headR2 = (socket_head_dia+tolerance)/2;
    //headRad = 
    translate([0, 0, -socket_head_thickness*.75])
    difference() {
      cylinder(r1 = headR1, r2 = headR2, h = headThick*.75);
      translate([0, 0, headThick*.75/2+o])
        // hex tool socket
        cylinder(r = hexRadius(socket_tool_size), h = headThick/2, $fn=6);
    }
  } // end if flatSocket
  ...
  if (head == "button") {
    c = socket_head_dia; // chord length 
    f = button_thickness*1.25; // height of button  * 1.25 rough aproximation of proper size 
    ...
    difference() {
      ...
      translate([0, 0, f/3])
      cylinder(r = hexRadius(socket_tool_size), h = f, $fn = 6);
...
module bolt(name="m3", threadType="", length=10, diameter=-1, head="socket",
            pitch=-1, hex_head_thickness=-1, hex_head_dia=-1, 
            socket_head_dia=-1, socket_head_thickness=-1, socket_tool_size=-1,
            button_thickness=-1,
    ...
    hex_head_thickness = override_default(hex_head_thickness, spec[4]);
    hex_head_dia       = override_default(hex_head_dia, spec[5]);
    socket_head_dia    = override_default(socket_head_dia, spec[6]);
    socket_head_thickness = override_default(socket_head_thickness, spec[7]);
    socket_tool_size   = override_default(socket_tool_size, spec[8]);
    button_thickness   = override_default(button_thickness, spec[10]);
    ...
    adjustedLength = (head == "flatSocket" || head == "flatHead") ? 
                     length - socket_head_thickness*.75 : length;
    
    if (v) {
        echo("thread: ", threadType);
        echo("diameter: ", diameter, " + ", tolerance);
        echo("pitch: ", pitch);
        echo("hex_head_thickness: ", hex_head_thickness);
        echo("hex_head_dia: ", hex_head_dia);
        echo("socket_head_dia: ", socket_head_dia);
        echo("socket_head_thickness: ", socket_head_thickness);
        echo("socket_tool_size: ", socket_tool_size);
        echo("button_thickness: ", button_thickness);
    }
    ...
    translate([0, 0, adjustedLength])
        boltHead(head, diameter, hex_head_thickness, hex_head_dia, 
                 socket_head_dia, socket_head_thickness, socket_tool_size,
                 button_thickness, tolerance, quality, list);
 }
 
 module nut(name="m3", threadType="", diameter=-1, pitch=-1,
            outer_dia=-1, thickness=-1, chamfer=true, slot=false,
    ...
    diameter    = override_default(diameter, spec[2]);
    pitch       = override_default(pitch, spec[3]);
    outer_dia   = override_default(outer_dia, spec[5]);
    ...
    radius = hexRadius(outer_dia+tolerance/2);
    boltSize = outer_dia+tolerance;
